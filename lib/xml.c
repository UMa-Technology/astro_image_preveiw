...
static struct xml_attribute **xml_find_attributes(struct xml_parser *parser, struct xml_string *tag_open) {
  ...
  const char *ptr = (const char *)tag_open->buffer;
  const char *end = ptr + tag_open->length;

  // Skip tag name (first word)
  while (ptr < end && !isspace(*ptr)) ptr++;
  
  // Update tag_open->length to exclude attributes
  tag_open->length = ptr - (const char *)tag_open->buffer;
  ...
  while (ptr < end) {
    ...
    // Find quote character (single or double)
    if (ptr >= end) break;
    char quote = *ptr;
    if (quote != '"' && quote != '\'') break;
    ptr++; // skip opening quote

    // Find value (until closing quote)
    const char *value_start = ptr;
    while (ptr < end && *ptr != quote) ptr++;
    if (ptr >= end) break;
    ...
    new_attribute->name->buffer = (unsigned char *)name_start;
    new_attribute->name->length = name_len;
    new_attribute->content = malloc(sizeof(struct xml_string));
    new_attribute->content->buffer = (unsigned char *)value_start;
    new_attribute->content->length = value_len;
